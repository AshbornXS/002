  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'remove':
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bem-vindo, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
        ...
        break
      case 'promote':
        text = (chat.sPromote || this.spromote || conn.spromote || '@user ```agora é um admin!```')
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```não é mais admin```')
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Este comando só pode ser usado pelo *Dono do Bot*',
    ...
    admin: 'Este comando é apenas para *Admins*!',
...
Seu número não está registrado no banco de dados
Use /registro Nome.Idade
*Exemplo* : /registro Frosty.20`
  }[type]
